			...
			if(count>=2)
			{
				...
				else if((mem_gv.mem_table[*idx+2].addr>>28) == 9)
				{
					...
					for(i=0;i<count;i++)
					{
						...
						adr_offset += temp;
						if((adr)>0x8800000 && (adr)<0xa000000){//âBõ¢ï≈öoò•ãcãwõµ
							switch(back)
							{
							case 0x0:
							case 0x3:
...
static int mem_add_result(unsigned int addr)
{
	...
	if(mem_search.count == RESULT_NUM)		//í^1024åÙñpñóèÅçõò≤ôπäÃó¬é¸
	{
...
static int mem_search_normal(unsigned int value, int (*eq_func)(unsigned int addr, unsigned int v), unsigned int low, unsigned int high, int apx)
{
	...
	case 0:
		...
		else		//ïnäÃñpñóèÅçõó≤äKçùRESULT_NUM,óäî©ä‰öÿíÿä‰êÌ
		{
