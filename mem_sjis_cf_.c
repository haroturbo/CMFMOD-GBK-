...
static void mem_write_value(int *idx)
{
	...
	u32 temp;
	u32 head=(codefreakdec(mem_gv.mem_table[*idx].addr)) >> 28;
	//char s[80];
	...
	case 0xC:
		if(mem_gv.mem_table[*idx].lock){
			adr = codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFFFFFFF;
			for(i=0;i<mem_gv.inst_num;i++){
	...
	case 0x8:
		if(mem_gv.mem_table[*idx].lock)
		{
			...
			temp = codefreakdec(mem_gv.mem_table[*idx+1].addr-0x8800000);
			if(temp < 0x10000000){
				val = temp & 0xFF;
	...
	case 0x4:
		if(mem_gv.mem_table[*idx].lock)
		{
			...
			val = codefreakdec(mem_gv.mem_table[*idx+1].addr)-0x8800000;
	...
	case 0xD:
		if((mem_gv.mem_table[*idx].value>>28) == 0x1){
			adr = (codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFF)+1;
			i=*idx+adr;
			if(mem_gv.mem_table[*idx].lock){
			...
			count = codefreakdec(mem_gv.mem_table[*idx+1].addr-0x8800000) + 1;
			i=*idx+count;
			if(mem_gv.mem_table[*idx].lock)
			{
				adr = (codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFFFFFFF);
	...
	case 0xE:
		temp = codefreakdec(mem_gv.mem_table[*idx].addr-0x8800000);
		count = (temp >>16) & 0xFF;
		val = temp & 0xFFFF;
		i=*idx+count;
	...
	case 0x5:
		if(mem_gv.mem_table[*idx].lock)
		{
			val = codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFFFFFFF;
			count = mem_gv.mem_table[*idx].value;
			adr = codefreakdec(mem_gv.mem_table[*idx+1].addr) & 0xFFFFFFF;
	...
	case 0x3:
		back = codefreakdec(mem_gv.mem_table[*idx].addr-0x8800000);
	...
	case 0x6:
		idxcount = *idx + 1;
		count = codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFFFF;
		if(mem_gv.mem_table[*idx].lock)
		{
			adr = codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFFFFFFF;
			...
			u32 adr_offset = mem_gv.mem_table[idxcount].value;
					back = codefreakdec(mem_gv.mem_table[idxcount].addr) >> 16;
					back = back & 0xF;
			if(count>=2)
			{
				temp = (codefreakdec(mem_gv.mem_table[*idx+2].addr- 0x8800000) & 0xFFFFFFF) ;
				u32 qqq = (codefreakdec(mem_gv.mem_table[idxcount].addr) >> 20) - 0x88;
				qqq *= 4;				
				if((codefreakdec(mem_gv.mem_table[*idx+2].addr)>>28) == 1)
				{
					back = adr + qqq;
					back = * ((u32 *)back);
					adr =  * ((u32 *)adr);
				...
				else if((codefreakdec(mem_gv.mem_table[*idx+2].addr)>>28) == 9)
				{
				...
				{			
					for(i=2;i<=count;i++)
					{
						...
						if((i&1)){
							...
							temp = codefreakdec(mem_gv.mem_table[ic].addr)>>28;
							adr_step = (codefreakdec(mem_gv.mem_table[ic].addr)-0x8800000)&0x0FFFFFFF;
						}
							adr =  * ((u32 *)adr);
						if(adr>0x8800000 && adr<0xa000000)
						{
...
PTROUT:
		if(count>=2){
			u32 flag;
			flag = codefreakdec(mem_gv.mem_table[*idx+2].addr) >> 28;
	...
	case 0xF:
		idxcount = (codefreakdec(mem_gv.mem_table[*idx].addr) & 0xFF);
		...
		if(mem_gv.mem_table[*idx].lock)
		{
			...
			{
				ptr_exec->adr = codefreakdec(ptr_code->addr) - 0x08800000;
