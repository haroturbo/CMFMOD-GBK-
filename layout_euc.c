...
static void layout_table_printname(p_mem_table table, int idx)
{
	...
	sname[10]=0;
	sprintf((char *)&g_text_array[idx][0], "%-10s 0x%08X 0x%08X %-4s%4s", sname, table[idx].addr - 0x08800000, table[idx].value, menu_change[table[idx].type], menu_yesno[1 - table[idx].lock]);
...
table_summary:
	...
	while(1)
	{
		...
		for(i = 0; i < layout_gv.tc; i ++){
			...
			s[10] = 0;
			sprintf((char *)&text_array[i][0], "%-11s0x%08X 0x%08X %-4s%4s", s, table[i].addr - 0x08800000, table[i].value, menu_change[table[i].type], menu_yesno[1 - table[i].lock]);
		}
		layout_table_display(layout_table_detail);
